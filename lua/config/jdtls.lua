local function get_jdtls_base_path()
...
local function get_workspace()
	...
	local workspace_path = "/opt/workspace/"
...
local function java_keymaps()
	...
	-- Set a Vim motion to <Space> + <Shift>J + <Shift>C to extract the code under the cursor to a method
	vim.keymap.set(
		"n",
		"<leader>jem",
		"<Cmd> lua require('jdtls').extract_method()<CR>",
		{ desc = "[j]ava [e]xtract [m]ethod" }
	)
	-- Set a Vim motion to <Space> + <Shift>J + <Shift>C to extract the code selected in visual mode to a static variable
	vim.keymap.set(
		"v",
	...
	-- Compile Incremental
	vim.keymap.set("n", "<leader>jbi", "<Cmd> JdtCompile incremental<CR>", { desc = "[j]ava [b]uild [i]ncremental" })
	-- Compile Full
	vim.keymap.set("n", "<leader>jbf", "<Cmd> JdtCompile full<CR>", { desc = "[j]ava [b]uild [f]ull" })
end
...
local function setup_jdtls()
	...
	-- Disable LSP semantic tokens since they mess with syntax color schemes.
	-- (XXX) Probably can fix this by setting colors for semantic tokens but don't know this yet.
	...
	local jdtls_base_path = get_jdtls_base_path()
  ...
  -- (XXX) Finds root dir at the module level. This could be whatever Java source was opened.
	-- local root_dir = jdtls.setup.find_root({ ".git", "mvnw", "gradlew", "pom.xml", "build.gradle" })
  -- (XXX) Finds root dir based on the current working directory. This is usually the opened project.
	local root_dir = vim.fn.getcwd()
	print("[WARN] Root dir is: " .. root_dir)
	...
	local java_path = home .. "/.jenv/versions/17.0/bin/java"

	local data_path = home .. "/.jdtls-workspace"

	-- (XXX) Set the command that start JDTLS
	-- Requires Python and JDK 17.
	-- Set JDK 17 to the jenv alias version "17.0".
	local cmd = {
		jdtls_base_path .. "/bin/jdtls",
		"--java-executable",
		java_path,
		"--jvm-arg=-Xmx2g",
		"--jvm-arg=-javaagent:" .. jdtls_base_path .. "/lombok.jar",
		"-data",
		data_path,
	}
		...
		java = {
			...
			configuration = {
				...
				runtimes = {
					...
					{
						name = "JavaSE-21",
						path = home .. "/.jenv/versions/21.0",
					},
					{
						name = "JavaSE-23",
						path = home .. "/.jenv/versions/23.0",
					},
				},
			},
			references = {
